...
export default function TeacherThreadPage() {
  ...
  const threadQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/teacher/messages/${threadId}`);
      if (!res.ok) throw new Error("Errore");
      return (await res.json()) as { thread: { threadId: string; subject: string | null }; messages: Message[] };
    },
    staleTime: 10_000,
  });

  const messages = threadQuery.data?.messages ?? [];
  const subject = threadQuery.data?.thread?.subject;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-10rem)]">
      <div className="mb-4">
        <Link href="/docente/messaggi" className="text-xs text-primary">← Torna ai messaggi</Link>
        <h1 className="mt-1 text-lg font-semibold">{subject || "Conversazione"}</h1>
      </div>
          ...
          messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${isTeacher ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${isTeacher ? "bg-amber-50 border border-amber-200" : "bg-white border border-gray-200"}`}>
                  <div className="flex items-center justify-between gap-3 mb-1">
                    <span className="text-xs font-medium">{isTeacher ? "Tu" : m.senderName || "Admin"}</span>
