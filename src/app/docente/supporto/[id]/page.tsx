...
async function fetchTicket(ticketId: string) {
  const response = await fetch(`/api/teacher/tickets/${ticketId}`);
  if (!response.ok) {
...
export default function TeacherSupportoDetailPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["tickets", "teacher-detail", params.id],
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/teacher/tickets/${data.id}/messages`, {
        method: "POST",
        body: formData,
      });
      ...
      await queryClient.invalidateQueries({ queryKey: ["tickets", "teacher-detail", data.id] });
      await queryClient.invalidateQueries({ queryKey: ["tickets", "teacher"] });
    } catch {
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <span
                className={`rounded-full px-2 py-0.5 font-medium ${getPriorityClasses(
                  data.priority
                )}`}
              >
      ...
      {isClosed ? (
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
          ...
          <p>Questo ticket e stato chiuso. Non e possibile inviare nuovi messaggi.</p>
        </div>
      ) : null}
