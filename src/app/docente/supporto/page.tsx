...
function NewTicketModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/teacher/tickets", {
        method: "POST",
        body: formData,
      });
          ...
          <div className="modal-body modal-scroll space-y-4">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Es. Problema accesso al registro"
                value={subject}
                maxLength={200}
...
async function fetchTickets(search?: string) {
  ...
  const response = await fetch(`/api/teacher/tickets?${params.toString()}`);
  if (!response.ok) {
...
export default function TeacherSupportoPage() {
  ...
  const { data, isLoading, isError, isFetching } = useQuery({
    queryKey: ["tickets", "teacher", debouncedSearch],
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filteredTickets.map((ticket) => (
            <Link
              key={ticket.id}
              href={`/docente/supporto/${ticket.id}`}
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="mb-2 flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${getPriorityClasses(
                        ticket.priority
                      )}`}
                    >
      ...
      <NewTicketModal
        ...
        onCreated={(ticketId) => {
          ...
          router.push(`/docente/supporto/${ticketId}`);
        }}
      />
    </div>
  );
