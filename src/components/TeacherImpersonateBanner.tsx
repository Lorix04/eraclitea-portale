...
export default function TeacherImpersonateBanner() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      const response = await fetch("/api/admin/impersonate-teacher/status", {
        cache: "no-store",
      });
      if (!response.ok) {
  ...
  const handleStopImpersonation = async () => {
    try {
      setStopping(true);
      await fetch("/api/admin/impersonate-teacher/stop", { method: "POST" });
    } finally {
      window.location.href = "/admin/docenti";
    }
  };
      ...
      <div className="fixed inset-x-0 top-0 z-[10001] border-b border-amber-300 bg-amber-400 px-4 py-2 text-amber-950 shadow-sm">
        <div className="mx-auto flex max-w-screen-2xl flex-wrap items-center justify-center gap-2 text-center text-xs font-medium md:gap-3 md:text-sm">
          ...
          <span>
            ...
            <strong>{status.teacherName || "Docente"}</strong> (Docente)
          </span>
          <button
            type="button"
            disabled={stopping}
